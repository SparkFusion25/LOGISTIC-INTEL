...
import { 
  ...
  Menu, X, Eye, AlertCircle, CheckCircle, Loader
} from 'lucide-react';
...
const SearchPanelDemo = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setCompanies([]);
        addNotification(data.error || 'Search failed', 'error');
      }
    } catch (error) {
  ...
  const handleAddToCRM = async (company: Company) => {
    ...
    try {
      ...
      if (result.success) {
        addNotification(`${company.company_name} added to CRM!`, 'success');
        ...
        if (userPlan === 'pro' || userPlan === 'enterprise') {
          try {
            ...
            addNotification('Contact enrichment queued', 'info');
          } catch (enrichError) {
...
📊 Trade Summary:
• Total Shipments: ${company.total_shipments}
• Total Value: $${(company.total_value_usd / 1000000).toFixed(1)}M
• Primary Mode: ${company.shipment_mode.toUpperCase()}
• Latest Activity: ${company.last_arrival}
...
Trade Intelligence Team`;
  ...
  const getModeColor = (mode: 'ocean' | 'air' | 'mixed') => {
              ...
              <div className="flex flex-wrap gap-2">
                {['all', 'ocean', 'air'].map(mode => (
                  <button
                    key={mode}
                    onClick={() => {
                      if (mode === 'air' && userPlan === 'trial') {
                        addNotification('Air shipment filtering requires Starter plan or higher', 'error');
                        return;
                      }
                      setFilterMode(mode);
                    }}
                  ...
                  >
                    ...
                    {mode === 'air' && (
                      <span className="flex items-center gap-2">
                        ...
                        Air {userPlan === 'trial' && <Lock className="w-3 h-3" />}
                      </span>
                    )}
                  </button>
                ))}
                  ...
                  <div className="p-4 lg:p-6">
                    ...
                    <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 mb-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <button 
                          ...
                          disabled={operationLoading[`crm_${company.company_name}`]}
                        ...
                        <button 
                          ...
                          disabled={!planFeatures.hasEmail || operationLoading[`email_${company.company_name}`]}
                        ...
                        >
                          {operationLoading[`email_${company.company_name}`] ? (
                            ...
                            <>
                              ...
                              {!planFeatures.hasEmail && <Lock className="w-3 h-3" />}
                            </>
                          )}
                          Send Insight
                        </button>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mb-4">
                      <div className="bg-gray-50 rounded-lg p-3">
                        ...
                        <p className="text-lg lg:text-xl font-bold">{company.total_shipments}</p>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-3">
                        <div className="flex items-center gap-2 text-gray-600 mb-1">
                        ...
                        <p className="text-lg lg:text-xl font-bold">${(company.total_value_usd / 1000000).toFixed(1)}M</p>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-3">
                        <div className="flex items-center gap-2 text-gray-600 mb-1">
                        ...
                        <p className="text-lg lg:text-xl font-bold">{(company.total_weight_kg / 1000).toFixed(0)}T</p>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-3">
                        <div className="flex items-center gap-2 text-gray-600 mb-1">
                        ...
                        <p className="text-xs lg:text-sm font-medium">{company.last_arrival}</p>
                      </div>
                    </div>
                    
                    {/* Contacts Section */}
                    {planFeatures.hasContacts ? (
                      company.contacts && company.contacts.length > 0 ? (
                        <div className="bg-green-50 border border-green-200 rounded-lg p-3 lg:p-4 mb-4">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <button
                              ...
                              className="text-green-600 text-xs lg:text-sm flex items-center gap-1"
                            >
                            ...
                            <div className="mt-3 space-y-2">
                              {company.contacts.map((contact, idx) => (
                                <div key={idx} className="bg-white rounded p-3">
                                  <p className="font-medium text-sm lg:text-base">{contact.full_name}</p>
                                  <p className="text-xs lg:text-sm text-gray-600">{contact.title}</p>
                                  <div className="flex flex-col lg:flex-row lg:items-center gap-1 lg:gap-3 mt-1">
                                    <span className="text-xs lg:text-sm text-indigo-600 flex items-center gap-1">
                        ...
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 lg:p-4 mb-4">
                          <span className="text-yellow-800 text-sm lg:text-base">
                            No contacts enriched yet - Click "Add to CRM" to enrich
                          </span>
                        </div>
                      )
                    ) : (
                      <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 lg:p-4 mb-4">
                        <div className="flex items-center gap-2 text-gray-600">
                          <Lock className="w-4 h-4" />
                          <span className="text-sm lg:text-base">
                            Contact enrichment requires Pro plan or higher
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="border-t pt-4">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 lg:gap-4">
                        <div className="bg-blue-50 rounded-lg p-3">
                          <p className="text-xs lg:text-sm text-blue-600 font-medium">Monthly Avg</p>
                          <p className="text-sm lg:text-lg font-bold">{Math.round(company.total_shipments / 12)} shipments</p>
                        </div>
                        <div className="bg-green-50 rounded-lg p-3">
                          <p className="text-xs lg:text-sm text-green-600 font-medium">Quarterly Value</p>
                          <p className="text-sm lg:text-lg font-bold">${(company.total_value_usd / 4 / 1000000).toFixed(1)}M</p>
                        </div>
                        <div className="bg-purple-50 rounded-lg p-3">
                          <p className="text-xs lg:text-sm text-purple-600 font-medium">Growth</p>
                          <p className="text-sm lg:text-lg font-bold">+{Math.round(Math.random() * 30 + 10)}%</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full mt-4 py-2 text-xs lg:text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg flex items-center justify-center gap-2"
                    >
                  ...
                  {expandedCompanies.has(company.company_name + '_details') && (
                    <div className="border-t bg-gray-50 p-4 lg:p-6">
                      <div className="overflow-x-auto">
                        <table className="w-full min-w-[500px]">
                          <thead>
                            <tr className="text-left text-xs lg:text-sm text-gray-600">
                              ...
                              <th className="pb-2 hidden lg:table-cell">Vessel</th>
                              <th className="pb-2">Value</th>
                            </tr>
                          </thead>
                          <tbody>
                            {company.shipments?.slice(0, 5).map((shipment, idx) => (
                              <tr key={idx} className="border-t">
                                <td className="py-2 text-xs lg:text-sm font-mono">{shipment.bol_number || 'N/A'}</td>
                                <td className="py-2 text-xs lg:text-sm">{shipment.arrival_date}</td>
                                <td className="py-2 text-xs lg:text-sm">
                                  {shipment.port_of_lading || shipment.port_of_loading} → {shipment.port_of_discharge}
                                </td>
                                <td className="py-2 text-xs lg:text-sm hidden lg:table-cell">{shipment.vessel_name || 'N/A'}</td>
                                <td className="py-2 text-xs lg:text-sm font-medium text-green-600">
                        ...
                        {company.shipments?.length > 5 && (
                          <p className="text-xs text-gray-500 mt-2">
                            Showing 5 of {company.shipments.length} shipments
                          </p>
                        )}
                      </div>
                    </div>
                ...
                <tbody className="divide-y">
                  {companies.map(company => (
                    <tr key={company.company_name} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-4 lg:px-6 text-xs lg:text-sm">{company.total_shipments}</td>
                      ...
                      <td className="px-3 py-4 lg:px-6 text-xs lg:text-sm hidden lg:table-cell">
                        {(company.total_weight_kg / 1000).toFixed(0)}T
                      </td>
                      ...
                      <td className="px-3 py-4 lg:px-6">
                        <div className="flex flex-col lg:flex-row gap-1 lg:gap-2">
                          ...
                          <button 
                            onClick={() => handleSendInsight(company)}
                            disabled={!planFeatures.hasEmail}
