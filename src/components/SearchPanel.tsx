...
// If not, change them to relative paths like "./InteractiveShipmentMap" etc.
...
export default function SearchPanel() {
  ...
  const fetchPage = async (reset = false) => {
    ...
    try {
      ...
      // swallow network errors for now (marketing surface)
      setHasMore(false);
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-lg shadow px-6 py-4 flex flex-wrap gap-4 items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:border-accent focus:ring-accent/50 outline-none"
          />
        </div>
