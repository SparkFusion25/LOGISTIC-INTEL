...
export async function POST(request: NextRequest) {
  try {
    console.log('🔧 Creating simple trade_data_view...');

    // Create a simple view using only existing columns
    ...
    if (error) {
      ...
      return NextResponse.json({
        success: false,
        error: 'Cannot execute SQL directly via RPC',
        details: error.message,
        alternative: 'Manual SQL execution required',
        testQuery: testError ? null : testQuery,
        sqlToRun: `
          DROP VIEW IF EXISTS public.trade_data_view;
    ...
    return NextResponse.json({
      success: true,
      message: 'Simple trade_data_view created successfully',
      data
    });
...
export async function GET() {
  return NextResponse.json({
    message: 'Simple view fix endpoint',
    description: 'Creates trade_data_view using only existing columns'
  });
}